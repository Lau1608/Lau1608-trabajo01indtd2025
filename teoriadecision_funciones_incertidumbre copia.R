...
#introducimos los datos por filas
...
# devuelve la posición en la que se encuentra el mínimo o los mínimos en caso de empate
# (la función de R wich.min devuelve sólo la primera posición en caso de empate)
...
# Si cual == 2: Crea una tabla con crea.tablaX(...) usando 9 valores y define que hay 3 alternativas y 3 estados.
# Si cual == 3: Crea una tabla con 6 valores y define 3 alternativas y 2 estados.
# Si no es 2 ni 3 (o sea, cual == 1): Crea una tabla con 12 valores y define 4 alternativas y 3 estados.
...
## factor de optimismo   (alfab * "lo mejor" Altmax en favor. y Altmin en desf.)
# Si es un número mayor a 1 → lo interpreta como número de divisiones del intervalo [0,1], y calcula Hurwicz para varios α.
...
# FUNCION : esta funcion genera un gráfico visual del criterio de Hurwicz para todas las alternativas según el factor de optimismo α.
dibuja.criterio.Hurwicz = function(tablaX,favorable=TRUE) {
  X = tablaX;
  Altmin = apply(X,MARGIN=1,min); # peor resultado de cada alternativa
  Altmax = apply(X,MARGIN=1,max); # mejor resultado de cada alternativa
  valfa = seq(from=0,to=1,by=0.05); # vector de valores para alfa (entre 0 y 1) en pasos de 0.05
  vHurwicz = rep(0,length(valfa));
  Alt_vHurwicz = rep(0,length(valfa));

  # Para cada α, calcula la combinación lineal entre mínimo y máximo.
  for (i in 1:length(valfa)) {
    alfab = valfa[i];
    if (favorable) {
      vAltH = alfab * Altmax + (1-alfab) * Altmin;
      vHurwicz[i] = max(vAltH) # vHurwicz[i] guarda el valor óptimo de Hurwicz (máximo en beneficios, mínimo en costes).
    } else {
  ...
  # Crea un lienzo vacío con eje x = α y eje y = valores Hurwicz y se ajustan el rango y los márgenes
  x0=0;x1=1;
  y0 = min(Altmin);
  y1 = max(Altmax);
  rg = y1-y0;
  ...
  # colores y lúneas base
  nn = length(Altmin);
  colores = rainbow(nn); # colores diferentes para cada alternativa (poisble fallo de que salgan todas azules)
  abline(v=0); # líneas verticales en α=0 y α=1
  abline(v=1);

  # para cada alternativa, dibuja la recta que une el mínimo (α=0) y el máximo (α=1)
   if (favorable) {
    for (i in 1:nn) {
      aa = Altmin[i];
  ...
  # dibuja la línea discontinua que une los valores óptimos de Hurwicz para cada α
  lines(valfa,vHurwicz,col=rainbow(nn+1)[nn+1],lty=3,lwd=3)

  # leyendas y título
  if (favorable) {
    legend("bottomright",legend=rownames(X),fill=colores,inset=0.05) # muestra las alternativas
    title("Criterio de Hurwicz (favorable - línea discontinua)") # título según favorable o desfavorable
  } else {
